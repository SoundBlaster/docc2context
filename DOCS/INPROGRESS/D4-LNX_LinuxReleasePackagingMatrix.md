...
- Release workflow now runs Linux `x86_64` and `aarch64` jobs plus macOS arm64, installing `rpm`/`dpkg-dev` dependencies before invoking the packaging script. Each job uploads tarballs, packages, checksums, and markdown summaries to the GitHub Release. The ARM lane uses the GitHub-hosted `ubuntu-22.04-arm64` runner tier (or a self-hosted `linux/arm64` runner when that image is unavailable).
