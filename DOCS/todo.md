...
- [ ] **C1 Markdown Snapshot Specs** Author failing Markdown snapshot tests plus fixture layout covering tutorial volumes, chapters, and articles before renderer code begins. _Depends on:_ B6 internal model + serialization readiness, C1 outline. _Doc:_ PRD §Phase C, workplan §Phase C. _Owner:_ docc2context agent. _Status:_ In Progress – documenting `MarkdownSnapshotSpecsTests.test_tutorialOverviewMatchesSnapshot` plus snapshot directory plans inside `DOCS/INPROGRESS/C1_MarkdownSnapshotSpecs.md` before writing renderer code.
