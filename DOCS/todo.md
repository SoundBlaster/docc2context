...
## In Progress
- [ ] **B6 Serialization Coverage** Add JSON serialization + determinism tests for `DoccBundleModel` and tutorial volumes. _Depends on:_ B6. _Doc:_ PRD §Phase B acceptance criteria. _Owner:_ docc2context agent. _Status:_ In Progress – see [`DOCS/INPROGRESS/B6_SerializationCoverage.md`](./INPROGRESS/B6_SerializationCoverage.md). First failing test: `DoccInternalModelSerializationTests.test_bundleModelIsCodable` to assert Codable conformance before building deterministic encoders.
