...
- [ ] **B3** Detect input type for directories vs `.doccarchive` inputs so downstream parsing always receives normalized bundle paths. _Depends on:_ B2. _Doc:_ PRD §Phase B. _Owner:_ docc2context agent. _Status:_ Newly selected – tracked in `DOCS/INPROGRESS/B3_InputDetection.md` with plan + heuristics.
