...
- [ ] **B3** Detect input type for directories vs `.doccarchive` inputs so downstream parsing always receives normalized bundle paths. _Depends on:_ B2. _Doc:_ PRD §Phase B. _Owner:_ docc2context agent. _Status:_ In Progress – START ritual captured in [`DOCS/INPROGRESS/B3_InputDetection.md`](DOCS/INPROGRESS/B3_InputDetection.md) with validation plan + test scaffolding (`InputDetectionTests`).
- [ ] **D1** Implement logging & progress instrumentation so the CLI reports detection/extraction/generation phases with snapshot-tested structure. _Depends on:_ B2. _Doc:_ PRD §Phase D. _Owner:_ docc2context agent. _Status:_ In Progress – START 2024-03-10 with updated plan + `LoggingTests` scaffolding documented in [`DOCS/INPROGRESS/D1_StructuredLogging.md`](DOCS/INPROGRESS/D1_StructuredLogging.md).
