...
- **C1 Author Snapshot Specs for Markdown Output** — golden Markdown fixtures for each DocC entity type. Planning outline archived under `DOCS/TASK_ARCHIVE/14_C1_MarkdownRenderingStrategy/`, and the tutorial overview + first chapter snapshots are archived under `DOCS/TASK_ARCHIVE/15_C1_TutorialChapterSnapshot/`; reference article specs remain under the active TODO entry.
