...
public struct DoccMetadataParser {
    ...
    public func loadSymbolGraphReferences(from bundleURL: URL) throws -> [DoccSymbolReference] {
        ...
        for fileURL in fileURLs where fileURL.lastPathComponent.lowercased().hasSuffix(".symbols.json") {
...
extension DoccMetadataParser {
    private func logSkippedSymbols(count: Int, fileURL: URL) {
        let message = "DoccMetadataParser: skipped \(count) symbol(s) in \(fileURL.lastPathComponent) due to missing identifiers or titles.\n"
