...
The GitHub Actions workflow at `.github/workflows/release.yml` now runs a Linux matrix for `x86_64` and `aarch64` plus a macOS arm64 job. The `x86_64` runner installs Swift 6.1.2 natively, while the `aarch64` variant executes under [`uraimo/run-on-arch-action`](https://github.com/uraimo/run-on-arch-action) to emulate an Ubuntu 22.04 ARM host (GitHub does not yet offer hosted ARM runners). Each job installs the platform prerequisites (`dpkg-deb`, `rpmbuild`, `zip`, and Swift 6.1.2), runs the packaging script with the appropriate `--arch`, uploads the Linux tarballs/packages alongside macOS zips, and publishes every archive + checksum to the GitHub Release so downstream automation can fetch binaries deterministically.
